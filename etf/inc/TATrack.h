...
public:
	...
	double GetDsquare();
	double GetChi2();
	double GetChi(); ///< \return sqrt(chi2) / nFiredAnodeLayer
	double GetSlope();
	double GetIntercept();
	...
	/// for kNormalFit only.\n
	/// (xc, zc) is the perturbative rotating center of particle track line in an attempt to
	/// minimize chi2 and get the optimal combination of linear parameter set
