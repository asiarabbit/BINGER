...
double TAAnode::GetDriftDistance(double dt, int STR_id){ // k is the track slope
	...
	if(7 == detId){ // DCTA array, large drift cells - 10mm max drift distance
		static const double p[8] = {1.06832, 0.0953891, 0.000766847, -0.000123101, 3.42713e-06, -4.51743e-08, 2.92191e-10, -7.41515e-13};
		r_base = p[7]*pow(dt,7)+p[6]*pow(dt,6)+p[5]*pow(dt,5)+p[4]*pow(dt,4)+p[3]*dt*dt*dt+p[2]*dt*dt+p[1]*dt+p[0]; // DEBUG
		if(r_base < 0. || dt < -5.) r_base = 0.; // DEBUG
		if(r_base > 5. || dt > 120.) r_base = 5.; // DEBUG
	}
	if(6 == detId){ // DCTA array, large drift cells - 10mm max drift distance
		static const double p[8] = {0.342525, 0.0295802, 0.00132393, -4.08718e-05, 7.11152e-07, -1.2406e-08, 1.32445e-10, -5.28087e-13};
		r_base = p[7]*pow(dt,7)+p[6]*pow(dt,6)+p[5]*pow(dt,5)+p[4]*pow(dt,4)+p[3]*dt*dt*dt+p[2]*dt*dt+p[1]*dt+p[0]; // DEBUG
		if(r_base < 0. || dt < -5.) r_base = 0.; // DEBUG
		if(r_base > 5. || dt > 120.) r_base = 5.; // DEBUG
	}
