...
//																																							     //
// BINGER/inc/etf/TACtrlPara.cxx																								     //
//   TACtrlPara.cxx -- source file for class TACtrlPara													     //
...
// Created: 2017/10/7.																													     //
// Last modified: 2020/03/13, SUN Yazhou.																				     //
//																																							     //
//																																							     //
...
bool TACtrlPara::IsCheckBunchIdAlignment() const{ return false; }
bool TACtrlPara::IsDriftTimeQtCorrection() const{ return false; }
bool TACtrlPara::IsCoarseFit() const{ return kIsCoarseFit; }
bool TACtrlPara::Is3DTracking() const{ return kIs3DTracking; }
// tolerance window for 3D coincidence test of X U and V track projections
// 5: half a DC cell, given all kinds of errors
double TACtrlPara::Get3DCoincideWindow() const{ return gp->Val(40); }
...
double TACtrlPara::D2Thre(unsigned uid) const{
	if(999999999 != uid){
...
bool TACtrlPara::TimeThre(double t, unsigned uid) const{
	if(999999999 != uid){
		int type[6]; TAUIDParser::DNS(type, uid);
		if(6 == type[0]){ // DCArrU -> Medium sized DC with drift cell size to be 5mm
...
double TACtrlPara::ChiThrePD(unsigned uid) const{
	int type[6]{}; TAUIDParser::DNS(type, uid);
	if(IsCoarseFit()){
...
int TACtrlPara::Vicinity() const{
	return gp->Val(45);
} // used in discerning multiple tracks, unit: cell
int TACtrlPara::StripTolerance() const{ return gp->Val(46); } // used in discerning multi-trks, unit: strip
// TATrack::kBFGSFit; // kNormalFit: 0; kBFGSFit: 1 kIterFit: 2
int TACtrlPara::FitMethod() const{ return gp->Val(47); }
// only effective if input fit method is kNormalFit
// allowed value: -2, -1, 0, 1, 2, 3, with calculation depth increasing. unit: mm^2
int TACtrlPara::Precision() const{ return gp->Val(48); }
// MWDCArrayR_DC1_U: installation error
double TACtrlPara::DCArrR_DC1UHorizontalDeviation() const{ return 0.; } // 4.;
...
double TACtrlPara::DsquareThresholdPerDot(unsigned uid) const{
	int type[6]{}; TAUIDParser::DNS(type, uid);
...
double TACtrlPara::T_tofDCtoTOFW(unsigned uid) const{
	int type[6]{}; TAUIDParser::DNS(type, uid);
	if(3 != type[0] && 4 != type[0] && 6 != type[0] && 7 != type[0] && 8 != type[0] && 9 != type[0])
...
double TACtrlPara::T_wireMean(unsigned uid) const{
	int type[6]{}; TAUIDParser::DNS(type, uid);
	if(3 != type[0] && 4 != type[0] && 6 != type[0] && 7 != type[0] && 8 != type[0] && 9 != type[0])
...
double TACtrlPara::GetSimSpatialResolution() const{
	return fSigmaR;
}
