...
//																																							     //
// BINGER/inc/etf/TAPDCL.cxx																												 //
//   TAPDCL.cxx -- source file for class TAPDCL																			 //
...
TAPDCL::TAPDCL(const string &name, const string &title, unsigned uid)
		: TAMWDCTa(name, title, uid){
}
TAPDCL::~TAPDCL(){}

void TAPDCL::GetAnodeCenterPosition(int dcType, int layerOption, int anodeId, double *Ag) const{
	const int type = dcType, l = layerOption - 1;
	const int nAnoPL = GetNAnodePerLayer();
	if(0 != l && 1 != l)
		TAPopMsg::Error(GetName().c_str(),
			"GetAnodeCenterPosition: Invalid layerOption (only 1 or 2 is permitted): %d", layerOption);
