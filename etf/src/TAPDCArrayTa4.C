...
bool TAPDCArrayTa4::Map(){
 ...
 (pre-Tracking not done yet) for splined target-zone track amelioration.");
	...
	// the track list
	static vector<tTrack *> &tl = evProsr->GetTrackList();
...
//	const double chiThrePD = clp->ChiThrePD(UID); // chi threshold per dot
	...
	for(int ll = 0; ll <= nAnodePerLayer[1][3]; ll++){ nu[1][3] = -1; // DC3-X2
		...
		for(int I = 0; I < 2; I++){ // loop over DCs
			normalEvent[I] = nu[I][0] >= 0 && nu[I][1] >=0 && nu[I][2] >= 0 && nu[I][3] >= 0; // all the 4 sense wires layers are effectively fired
			...
			specialEvent[I] = inert[0] || inert[1] || inert[2] || inert[3]; // only one sense wire layer is inert
		} // end for over I: DCs
		...
		cout << endl;
		cout << "normalEvent[0]: " << normalEvent[0] << "\rspecialEvent[0]: " << specialEvent[0] << endl;
		getchar(); // DEBUG
		...
		cout << endl;
		cout << "normalEvent[1]: " << normalEvent[1] << "\rspecialEvent[1]: " << specialEvent[1] << endl;
...
// sleep(3);
	...
	if(track.size() > 1){
		...
		vector<TATrackTa4 *>::iterator it = remove_if(track.begin(), track.end(), [](TATrackTa4 *t){ return !(t->GetTitle() == "THE_ONE"); });
