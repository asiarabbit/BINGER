...
//																																							     //
// BINGER/inc/etf/TAT0CalibDCArr.cxx																								 //
//   TAT0CalibDCArr.cxx -- source file for class TAT0CalibDCArr											 //
...
#endif
							t[trkId[jj][l]][j] -= clp->T_tofDCtoTOFW(uid) - clp->T_wireMean(uid); // recover the rough correction of time of flight from DC to TOF wall for a refined one.
...
void TAT0CalibDCArr::GenerateCalibFile(const string &rootfile, TAMWDCArray *dcArr, bool isShowFit){
	...
	if(!access(name, F_OK)){ // TAParaManager::Instance()->Exist(2)
		...
		char cmd[128]; sprintf(cmd, "echo;echo ls %s/T0/*:; ls %s/T0/*;echo", clp->ConfigExpDir(), clp->ConfigExpDir());
		system(cmd);
	}
