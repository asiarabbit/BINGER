...
//																																							     //
// BINGER/inc/etf/TATrackTa4.cxx																										 //
//   TATrackTa4.cxx -- source file for class TATrackTa4															 //
...
void TATrackTa4::Fit(){
	...
	if(!trkPreTa->IsAssigned())
		TAPopMsg::Error(GetName().c_str(),
			"Fit: the track %s has not been assigned", trkPreTa->GetName().c_str());
	if(!trkPostTa->IsAssigned())
		TAPopMsg::Error(GetName().c_str(),
			"Fit: the track %s has not been assigned", trkPostTa->GetName().c_str());
	// initialize chi
	fChi2 = 0.;
...
//	k[1] = trkPostTa->GetSlope(); b[1] = trkPostTa->GetIntercept(); // will trigger fit
	...
	TAMath::IterFit4Single(z, x, r, k, b, gGOOD, LAYER, d2ThrePD); // a new one
