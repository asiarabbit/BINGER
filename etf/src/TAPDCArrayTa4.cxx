...
//																																							     //
// BINGER/inc/etf/TAPDCArrayTa4.cxx																									 //
//   TAPDCArrayTa4.cxx -- source file for class TAPDCArrayTa4												 //
...
TAMWDCArray2 *TAPDCArrayTa4::GetMWDCArray(int dcArrId) const{
	if(dcArrId != 0 && dcArrId != 1)
		TAPopMsg::Error(GetName().c_str(), "GetMWDCArray: dcArrId out of range, \
dcId: %d, only 0-dcArrL and 1-dcArrR are allowed", dcArrId);
	if(!fDCArr[dcArrId])
		TAPopMsg::Error(GetName().c_str(), "GetMWDCArray: requested dcArr pointer\
is null, dcArrId: %d", dcArrId);
...
void TAPDCArrayTa4::AssignTracks(vector<tTrack *> &track_ls){
	...
	if(!trkTa4) TAPopMsg::Error(GetName().c_str(), "AssignTracks: trkTa4 null, odd..."); // DEBUG
	TATrack2 *trk[2] = {trkTa4->GetTrackPreTa(), trkTa4->GetTrackPostTa()};
	// cout << "trk[0]: " << trk[0]->GetName(); // DEBUG
	// cout << " trk[1]: " << trk[1]->GetName() << endl; // DEBUG
	// getchar(); // DEBUG
...
//	const double chiThrePD = clp->ChiThrePD(UID); // chi threshold per dot
	...
	for(int j = 0; j <= nAnodePerLayer[0][0]; j++){ nu[0][1] = -1; // DC0-X2
		if(j < nAnodePerLayer[0][0] && fDC[0][0]->GetAnodeL2(0, j)->GetFiredStatus()) nu[0][1] = j; // DC0-X2 --------------------------------------------------------------
	...
	//	if(1 == gGOOD[0] || 1 == gGOOD[1]) continue;
		...
		getchar(); // DEBUG
#endif
		if(d2[0] < d2Thre * nFiredAnodeLayer[0] &&
			 d2[1] < d2Thre * nFiredAnodeLayer[1] && d2Extra < chi2ExtraThre * 3.0){
#ifdef DEBUG_MAP
			...
			for(int i = 0; i < 2; i++){
				...
				if(-9999. == TOF[i]){ // drift time start is not available
					isBadTrack = true;
					TAPopMsg::Error("TAPDCArrayTa4",
						"Map: Second time tracking, TOF invalid");
					continue;
				}
			...
			for(int i = 0; i < 2; i++){ // loop over the two DCTas
				newtrk[i]->SetData(x[i], z[i], t[i], r[i], kl[i], bl[i], d2[i],
					gGOOD[i], nu[i], LAYER[i]);
				newtrk[i]->SetTOF(TOF[i], -1, -9999.);
			} // end for over i


			//////// XXX XXX XXX XXX XXX XXX XXX XXX //////////
			//// // the magnificent majestic FIT method // ////
			newTrack.Fit();
			//////// XXX XXX XXX XXX XXX XXX XXX XXX //////////
			...
			for(int i = 0; i < 2; i++){ // loop over the two DCTas
				...
				getchar(); // DEBUG
#endif
				if(fabs(newtrk[i]->GetChi()) > chiThre[i]){
					isBadTrack = true;
					continue;
				}
//				cout << "mark 1" << endl; // DEBUG
				for(double cc : chi[i]){
					if(-9999. != cc && fabs(cc) > chiThrePD[i]){
						isBadTrack = true;
						continue;
					} // end inner if
...
track.size(): %d, overlap: %d_______________", track.size(), overlap);
			}
			if(cmpShow){
				TAPopMsg::Debug(GetName().c_str(), "map: Before pushback, \
track.size(): %d", track.size());
			} // end if // DEBUG
#endif
...
#ifdef DEBUG_MAP
			if(cmpShow){ // DEBUG
				TAPopMsg::Debug(GetName().c_str(), "map: After pushback, \
track.size(): %d", track.size());
			} // end if
...
int TAPDCArrayTa4::compare(TATrackTa4 *newTrack, TATrackTa4 *oldTrack, bool show){
	...
	if(show){ // DEBUG
		cout << "nValid_nu: " << nValid_nu << "\tnValid_nu_temp: "; // DEBUG
		cout << nValid_nu_temp << endl; // DEBUG
...
void TAPDCArrayTa4::FillTrack(TGraph *gTrack, TGraph *gTrack_R) const{
	...
	for(TATrackTa4 *t : fTrackTa4List){
		t->GetTrackPreTa()->FillTrack(gTrack, gTrack_R, 10); // 1: dot density fac
		t->GetTrackPostTa()->FillTrack(gTrack, gTrack_R, 10);
	}
