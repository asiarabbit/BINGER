...
//																				  																			   //
// BINGER/inc/etf/TAMWDCArray2/map.cxx												 											 //
//   map.cxx -- source file for method TAMWDCArray2::Map														 //
...
bool TAMWDCArray2::Map(TAMWDC **MWDC, vector<TATrack2 *> &track, int dcType){
	...
	for(int jj = 0; jj <= nAnodePerLayer1; jj++){ nu[3] = -1; // DC1-X2
		...
		if(-1 == nu[3] && jj < nAnodePerLayer1) continue;
			...
			if(!normalEvent && !specialEvent) continue;
#ifdef DEBUG_MAP
...
//			if(1 == gGOOD) continue; // TEST - 20180930-22:33 XXX XXX XXX
			...
			// Here it's Dsquare(z, x...), NOT Dsquare(x, z...)
			// because the coordinate system is different from those conventional ones now
