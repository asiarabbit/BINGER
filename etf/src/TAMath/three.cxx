...
//																				     //
// BINGER/inc/etf/TAMath/three.C													 //
//   three.C -- source file for method Dsquare3 and minid2_3						 //
//   Introduction: Definition of function double Dsquare3(...). Linear least square	 //
// method fitting for three points that belongs to three DCs and calculate their	 //
// fitting chi2s.																	 //
//																				     //
...
double Dsquare3(const double *x, const double *y, double &k, double &b, const int *LAYER, double DsqThrePerDot){	
	...
	for(int i = 0; i < n; i++){
		if(i == LAYER[0] || i == LAYER[1] || i == LAYER[2]){ // X1 of each MWDC // JULY THE FIRST //
			SUMx  += x[i];
			SUMy  += y[i];
			SUMxx += x[i] * x[i];
			SUMxy += x[i] * y[i];
...
double minid2_3(const double *x, const double *y, const double *r, double k, double b, const int *LAYER){
	double temp = 0;
	double d2 = 0.;
	for(int i = 0; i < 6; i++){
		if(i == LAYER[0] || i == LAYER[1] || i == LAYER[2]){ // X1 of each MWDC // JULY THE FIRST //
