...
//																				 																			     //
// BINGER/inc/etf/TAMWDCArray/map.cxx																								 //
//   map.cxx -- source file for method TAMWDCArray::Map															 //
...
//				getchar(); // DEBUG
				...
				if('X' == type || 'Y' == type){ // MWDC_X or MWDC_Y
					...
					// 0.7*nstripStray to pass stripThreTest
					TOF = GetTOFWall()->GetTime(kl, bl, nstripStray, firedStripId, t0, t1, t2, 0.6);
...
//							cout << "1, t[i]: " << t[i] << endl; getchar(); // DEBUG
							...
							t[i] += clp->T_tofDCtoTOFW(uid) - clp->T_wireMean(uid);
...
//					cout << "clp->ChiThre(): " << clp->ChiThre() << endl; getchar(); // DEBUG
					...
					// default value is 1. Here it's explicitly written out for emphasis
					TOF = GetTOFWall()->GetTime(newTrack.GetSlope(),
						newTrack.GetIntercept(), nstripStray, firedStripId, t0, t1, t2, 1.);
					if(-9999. == TOF){
						isBadTrack = true; continue;
					}
					for(double cc : chi){
