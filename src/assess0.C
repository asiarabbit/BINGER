// assess0.C -- assess track qualities after step 0 - pattern recognition
...
int main(int argc, char *argv[]){
	...
	dcArrL = (TAMWDCArray*)dec_vec[3]; // dc array L
	dcArrR = (TAMWDCArray*)dec_vec[4]; // dc array R
...
//			cout << "type[j]: " << type[j] << endl; getchar();
			if(type[j] == 110) has_X = true;
			if(type[j] == 111) has_U = true;
			if(type[j] == 112) has_V = true;
			if(type[j]/10 == 10) continue;
			effTot++;
//			cout << "_____________________effTot: " << effTot << endl;
			int dcType = type[j]%110;
			int STRid = 0;
			for(int l = 0; l < 6; l++){
				if(nu[j][l] != -1){
