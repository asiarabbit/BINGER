...
//	return 0;
	...
	sprintf(cmd, "rm %s/T0/*", usr->GetCtrlPara()->ConfigExpDir()); system(cmd);
	sprintf(cmd, "rm %s/STR_cor/*", usr->GetCtrlPara()->ConfigExpDir()); system(cmd);
...
//		getchar(); // DEBUG
	...
	for(int i = 0; i < 4; i++){ // STR iteration
		for(int j = 0; j < 2; j++){ // loop over the two MWDC arrays; 0: L; 1: R
			...
			// void EvalDCArr(int runId = 0, bool isDCArrR = true)
			ass->EvalDCArr(i, bool(j)); // the implementation of the assess
