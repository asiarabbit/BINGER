...
//struct SHM_VME *shmp;
//int shmid;
extern float sum,add,sum1;
extern void clear_sca_buff();
extern float tdc[32],adc[32],qdc[32],mtdc1A[16][10],mtdc2A[16][10]; 
...
/////////////////////////// input
    ...
    char on_shm[] ="online_shm_VME";
    if(online_interface_open(on_shm, &p_output_online_shm_interface)!=0)
    cout<<"open shm error"<<endl;
    int input_online_data[DEFAULT_INTERFACE_ONLINE_LEN]={0};
    int nstored=-1;
/////////////////////////// input 

////////////////////// output
      static online_shm_interface* p_output_online_shm_interface1;    // input interface 
    char on_shm1[] ="online_shm_VME1";
    online_interface_destroy(on_shm1, p_output_online_shm_interface1);
    if(online_interface_create(on_shm1, &p_output_online_shm_interface1)!=0)
    {
         cout<<"online_shm error 1 "<<endl;
    }
    
    if(online_interface_open(on_shm1, &p_output_online_shm_interface1)!=0)
    {
         cout<<"online_shm error 2"<<endl;
    }

    int output_online_data1[DEFAULT_INTERFACE_ONLINE_LEN]={300};
...
////////////////////// output
  ...
  {  
    ...
    if(time_current_event!=time_last_event)
    {
      ...
      if(event_counts_per_sec != 0)
 {event_counts_per_pulse+=event_counts_per_sec;cout<<"    "<<setw(4)<<event_counts_per_pulse<<endl;}      
     ...
     if(nstored != 0)
     {
         ...
         //cout<<"Event Num: "<<input_online_data[0]<<endl;
         event_num++;
         event_num1= input_online_data[1];
...
////////////////output
      output_online_data1[0]=2;
      output_online_data1[1]=event_num1;
      output_online_data1[2]=8.9/130*tdc[0];
      int nempty1=-1;
      sem_getvalue(&p_output_online_shm_interface1->nempty,&nempty1);

      if(nempty1!=0)
        if(online_interface_add_task(&output_online_data1[0],p_output_online_shm_interface1)!=0)
        {
...
/*
     ...
     unsigned int analys_block, event_length,event_num=0,total_event, pos;
	extern long sbuff[16];
...
L0:
	...
	while(setonline)     // 1st while
	{
			...
			{    int proc=0;//cout<<endl<<"p3 "<<pqr++<<endl;
				...
				while (event_num == shmp->event_no-1)
                     {   usleep(100);
                         ...
                         proc++;if(proc%1000==0){gSystem->ProcessEvents(); }
                           ...
                           if(time_current_event!=time_last_event && time_current_event%10==0)
                           cout<<endl<<"Event num: "<<event_num<<endl;
				...
				while (event_num == shmp->event_no)  {usleep(100);
                    proc++;if(proc%1000==0){gSystem->ProcessEvents(); }}
				while (event_num == shmp->event_no-1){usleep(100);
                    proc++;if(proc%1000==0){gSystem->ProcessEvents(); }}
			...
			event_num = shmp->buffer[block_num].data[pos+1]; 
