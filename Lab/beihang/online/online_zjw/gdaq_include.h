...
#define DEFAULT_INTERFACE_ONLINE_NUM       	20000
#define DEFAULT_INTERFACE_ONLINE_LEN       	1000
...
typedef struct online_shm_interface_t{
    ...
    int     p_write, p_read;        
    int     section[DEFAULT_INTERFACE_ONLINE_NUM][DEFAULT_INTERFACE_ONLINE_LEN];         
...
int online_interface_create(const char shm_name[], online_shm_interface** p_interface);
int online_interface_open(const char shm_name[], online_shm_interface** p_interface);
int online_interface_destroy(const char shm_name[], online_shm_interface* p_interface);
...
int online_interface_create(const char shm_name[], online_shm_interface** p_interface)
{
	...
	*p_interface = (online_shm_interface*)(shm_int.ptr);
    ...
    if(sem_init(&(*p_interface)->nempty,1, DEFAULT_INTERFACE_ONLINE_NUM)==-1)
    {
...
int online_interface_add_task(int* task_addr,online_shm_interface* p_interface)
{
    ...
    /*critical region*/
    memcpy(&p_interface->section[p_interface->p_write][0], task_addr, sizeof(int)*DEFAULT_INTERFACE_ONLINE_LEN);
    p_interface->p_write++;
    if(p_interface->p_write == DEFAULT_INTERFACE_ONLINE_NUM)
        p_interface->p_write = 0;
    #ifdef DAQ_DEBUG
...
int online_interface_get_task(online_shm_interface* p_interface, int* out_task_addr)
{
    ...
    /*critical region*/
    memcpy(out_task_addr, &p_interface->section[p_interface->p_read][0], sizeof(int)*DEFAULT_INTERFACE_ONLINE_LEN);
    p_interface->p_read++;
    if(p_interface->p_read>= DEFAULT_INTERFACE_ONLINE_NUM)
        p_interface->p_read = 0;
