...
//	G4Material *water = nist->FindOrBuildMaterial("G4_WATER");
	...
	G4Box *solidWirePlanePDC = new G4Box("WirePlanePDC", 0.5*pdcSizeXY, 0.5*pdcSizeXY, 0.08*mm);
	...
	new G4PVPlacement(0, G4ThreeVector(), fLogicalDipo, "Dipo", logicalWorld,
		false, 0, checkOverlaps);
	fLogicalDipo->SetUserLimits(new G4UserLimits(1.*m));
	...
	G4Box *solidWirePlaneDC = new G4Box("WirePlaneDC", 0.5*dcSizeX, 0.5*dcSizeY, 0.1*mm);
