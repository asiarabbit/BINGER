...
const double dx2Mean = -2.202;

void dx2ratioLoop1DL(){
	const double dx2_0[2] = {-250., dx2Mean}, dx2_1[2] = {-1.5, 8.};
	...
	g2[0]->SetNameTitle("gdx2RatioC15", "ratio v.s. dx2 distributions - C15");
	g2[1]->SetNameTitle("gdx2RatioC14", "ratio v.s. dx2 distributions - C14");
	g2[2]->SetNameTitle("gdx2RatioC13", "ratio v.s. dx2 distributions - C13");
	g2[3]->SetNameTitle("gdx2RatioC12", "ratio v.s. dx2 distributions - C12");
	...
	TFile *f = new TFile("dx2_1D_1000LL.root", "recreate");
...
void dx2ratio(const double *dx2, const bool sGap, double *nC, double *ratioC){
	...
	// stored cuts //
	vector<const char*> nclLs = {"C15", "C14", "C13", "C12"};
	vector<TCutG *> cutgLs;
