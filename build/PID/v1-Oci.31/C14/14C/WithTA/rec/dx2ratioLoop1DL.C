...
const bool sGap = false;
const double dx2Mean = -2.468;

void dx2ratioLoop1DL(){
	const double dx2_0[2] = {-25., dx2Mean}, dx2_1[2] = {-1.5, 8.};
	const int dx2_nStep = 1000;
	...
	for(int i = size; i--;) g2[i] = new TGraph2D();
	g2[0]->SetNameTitle("gdx2RatioC14", "ratio v.s. dx2 distributions - C14");
	g2[1]->SetNameTitle("gdx2RatioC13", "ratio v.s. dx2 distributions - C13");
	g2[2]->SetNameTitle("gdx2RatioC12", "ratio v.s. dx2 distributions - C12");

	// Loop begins
	int point[size]{};
	for(int i = 0; i < dx2_nStep; i++){
		dx2[0] = dx2_0[0] + i * dx2_0Step;
		dx2[1] = 2. * dx2Mean - dx2[0];
	...
	TFile *f = new TFile("dx2_1D_1000L.root", "recreate");
...
void dx2ratio(const double *dx2, const bool sGap, double *nC, double *ratioC){
	TFile *f = new TFile("~/pionExp2017/build/20180710_1014.dat_0.root", "update"); // 0920 0508
	...
	// stored cuts //
	vector<const char*> nclLs = {"C14", "C13", "C12"};
	vector<TCutG *> cutgLs;
