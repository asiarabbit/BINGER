...
int main(){
	...
	const ExpData e_dT(0.9191, 0.0058); // detecting efficiency
	...
	const ExpData CC(4516, sqrt(4516)); // charge-changing yield
	const ExpData NR(1194, sqrt(1194)); // neutron-removal yield
#ifdef CCCS
	...
	const ExpData CC_n(253, sqrt(253)); // charge-changing yield
	const ExpData NR_n(329, sqrt(329)); // neutron-removal yield
#ifdef CCCS
