...
void yield(){
	...
	TFile *f = new TFile("~/pionExp2017/build1/C12NOTA.root", "update");
	vector<TCutG *> cutgLs;

	const int n = 100; // number of points for TCutG in the dE1-aoz spectrum
	const char *nuclLs[] = {"C12", "C11", "C10", "B11", "B10", "Be7", "C12R", "C12U"};
	...
	vector<double> x0 = {1.94, 1.786, 1.636, 2.126, 1.938, 1.709}; // aoz
	vector<double> a  = {0.07, 0.055, 0.07, 0.07, 0.07, 0.07}; // aoz
	vector<double> y0 = {1.724, 1.740, 1.741, 1.260, 1.239, 1.00}; // dE1
	vector<double> b  = {0.20, 0.20, 0.20, 0.20, 0.20, 0.20}; // dE1
	// for incNucl_U and incNucl_R //
	double daoz = fabs(x0[0] - x0[1]), ddE1 = fabs(2.31 - 1.77); // N - C
	// incNucl_R
