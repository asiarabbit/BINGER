...
void tmp(){
	TFile *f = new TFile("~/pionExp2017/build1/C15NOTA.root", "update"); // 0920 0508
...
//	treeTrack->Draw("tof2[0]:aoz[0]>>h(500, 2., 3., 500, 55., 73.)", "dsca11==0&&A0&&B0&&C0&&dE1>1.0;aoz;tof2;", "col");
	// B0
...
//	treeTrack->Draw("t0_1Pos[1]:t0_1Pos[0]>>(500, -60., 60., 500, -60., 60.)", "dsca11==0&&PDCPos[1][1]!=-9999.&&!C0", "col")
	treeTrack->Draw("dE1:aoz[0]>>h(500, 1.5, 3.7, 500, 0., 4.5)", "index<486800&&dsca11==0&&A0&&B0&&C0;aoz;dE1 arb.;", "colbox");
...
//	B3->Draw("same");
	...
	vector<const char *> nclLs = {"C15", "C15Ori", "C14", "C13", "C12", "B14", "B13", "B12", "B11", "B10", "Be11", "Be10", "Be9", "C15R", "C15U"};
	for(const char *n: nclLs) cutgLs.push_back((TCutG*)f->Get(n));
	if(pidShow) for(TCutG *c : cutgLs){
		c->Draw("same");
		const double tmp = treeTrack->GetEntries((string("index<486750&&dsca11==0&&A0&&B0&&C0&&") + c->GetName()).c_str()); // &&(dx2[0]>-9&&dx2[0]<4)&&dsca[4]==1
		cout << c->GetName() << ": " << tmp << endl;
	}
	cout << "N0: " << treeTrack->GetEntries("index<486750&&dsca11==0&&A0&&B0&&C0") << endl; // &&(dx2[0]>-9&&dx2[0]<4)&&dsca[4]==1
