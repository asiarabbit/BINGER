...
// c.s. unit: mb
// The abscissa is the time sequence of the beam settings
...
//		int i = 16-j; // atomic number A
		...
		geffC->SetPoint(ii, i, effC[j].val);
		geffC->SetPointError(ii, 0., effC[j].err);
		geffB->SetPoint(ii, i, effB[j].val);
		geffB->SetPointError(ii, 0., effB[j].err);
		gsigma0->SetPoint(ii, i, sigma0[j].val);
		gsigma0->SetPointError(ii, 0., sigma0[j].err);
		gsigmacc->SetPoint(ii, i, sigmacc[j].val);
		gsigmacc->SetPointError(ii, 0., sigmacc[j].err);
		gsigman->SetPoint(ii, i, sigman[j].val);
		gsigman->SetPointError(ii, 0., sigman[j].err);
		gsigma2n->SetPoint(ii, i, sigma2n[j].val);
		gsigma2n->SetPointError(ii, 0., sigma2n[j].err);
		gsigmap->SetPoint(ii, i, sigmap[j].val);
		gsigmap->SetPointError(ii, 0., sigmap[j].err);
		gsigmanp->SetPoint(ii, i, sigmanp[j].val);
		gsigmanp->SetPointError(ii, 0., sigmanp[j].err);
	...
	gsigman->SetLineColor(5); gsigman->SetMarkerColor(5);
	...
	c->SetGrid();
	geffC->SetMinimum(0.85); geffC->SetMaximum(1.);
	geffC->GetXaxis()->SetTitle("A");
	geffC->GetYaxis()->SetTitle("eff"); geffC->GetYaxis()->SetTitleOffset(1.);
	geffC->Draw("APL");
