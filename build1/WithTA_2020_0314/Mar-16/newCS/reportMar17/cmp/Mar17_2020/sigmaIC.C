...
void sigmaIC(){
	...
	gsigmaR83->SetNameTitle("gsigmaR83_1", "reaction and interaction c.s.");
	gsigmaI240->SetNameTitle("gsigmaI240_1", "interaction c.s.");
	gsigmaI900->SetNameTitle("gsigmaI900_1", "interaction c.s.");
	...
	lg->AddEntry(gsigmaR83, "83MeV/u, Reaction C.S. - New");
	lg->AddEntry(gsigmaI240, "240MeV/u, Interaction C.S. - New");
	lg->AddEntry(gsigmaI900, "900MeV/u, Interaction C.S. - New");
	lg->SetY1(0.7);
	lg->Draw("same");

	c->Print("sigmaIC_1.pdf");
	c->Print("sigmaIC_1.root");

	TFile *f = new TFile("sigmaIC_1.root", "update");
	g->Write("", TObject::kOverwrite);
	gsigmaI240->Write("", TObject::kOverwrite);
	gsigmaI900->Write("", TObject::kOverwrite);
	lg->Write("", TObject::kOverwrite);
	f->Close();
	system("cp sigmaIC_1.root ..");
