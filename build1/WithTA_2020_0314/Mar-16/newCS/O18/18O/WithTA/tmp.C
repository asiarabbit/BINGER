...
void tmp(){
	TFile *f = new TFile("~/pionExp2017/build1/O18TA.root", "update"); // 0920 0508
...
//	treeTrack->Draw("dE0+(PDCPos[1][1]*(0.005*dE0-0.0016)):tof1>>h(500, 132., 140., 500, 0., 4.5)", "dsca11==0&&PDCPos[1][1]!=-9999.&&C0;tof1 [ns];dE0-corrected arb.;", "col");
...
//	const char *cutOpt[] = {"dE1>1.5", "dE1>1.2&&dE1<1.5", "dE1>0.94&&dE1<1.2", "dE1>0.71&&dE1<0.94", "dE1>0.5&&dE1<0.71"}; // O-N-C-B-Be
//	treeTrack->Draw("aoz[0]:TOFWPos[0]>>h(500, -700., 700., 500, 1.4, 3.)", "dsca11==0&&A0&&B0&&C0&&(dx2[0]>-9&&dx2[0]<4)&&dE1>0.71&&dE1<0.94;TOFWPosX [mm];aoz;", "colbox");
...
//	treeTrack->Draw("tof2[0]:aoz[0]>>h(500, 1.4, 2.6, 500, 63., 73.)", "dsca11==0&&A0&&B0&&C0&&dE1>2.75;aoz;tof2;", "col");
	treeTrack->Draw("dE1:aoz[0]>>h(500, 1.6, 2.8, 500, 0., 4.5)", "dsca11==0&&A0&&B0&&C0;aoz;dE1 arb.;", "colbox"); // 
...
//	B3->Draw("same");
	...
	vector<const char *> nclLs = {"O18", "O17", "O16", "O15", "N17", "N16", "N15", "N14", "N13", "C16", "C15", "C14", "C13", "C12", "C11", "B13", "B12", "B11", "B10", "Be7", "Be9", "Be10", "objL", "objR", "objD"};
	for(const char *n: nclLs) cutgLs.push_back((TCutG*)f->Get(n));
	if(pidShow) for(TCutG *c : cutgLs){
		c->Draw("same");
		const double tmp = treeTrack->GetEntries((string("dsca11==0&&A0&&B0&&C0&&dsca4==1&&(dx2[0]>-9&&dx2[0]<4)&&") + c->GetName()).c_str());
		cout << c->GetName() << ": " << tmp << endl;		
	}
	cout << "N0: " << treeTrack->GetEntries("dsca11==0&&A0&&B0&&C0&&dsca4==1&&(dx2[0]>-9&&dx2[0]<4)") << endl;
...
//	treeTrack->Draw("dE1:dE0>>(500, -1., 5., 500, -1., 5.)", "multi_PDC[0][0][0][0]==1&&multi_PDC[0][0][0][1]==1&&multi_PDC[0][0][1][0]==1&&multi_PDC[0][0][1][1]==1&&multi_PDC[0][1][0][0]==1&&multi_PDC[0][1][0][1]==1&&multi_PDC[0][1][1][0]==1&&multi_PDC[0][1][1][1]==1&&dsca11==0&&dsca10==1&&multi_PDC[1][0][0][0]==1&&multi_PDC[1][0][0][1]==1&&multi_PDC[1][0][1][0]==1&&multi_PDC[1][0][1][1]==1&&multi_PDC[1][1][0][0]==1&&multi_PDC[1][1][0][1]==1&&multi_PDC[1][1][1][0]==1&&multi_PDC[1][1][1][1]==1&&multi_DC[1][0][0][0]==1&&multi_DC[1][0][0][1]==1&&multi_DC[1][0][1][0]==1&&multi_DC[1][0][1][1]==1&&multi_DC[1][0][2][0]==1&&multi_DC[1][0][2][1]==1&&multi_DC[1][1][0][0]==1&&multi_DC[1][1][0][1]==1&&multi_DC[1][1][1][0]==1&&multi_DC[1][1][1][1]==1&&multi_DC[1][1][2][0]==1&&multi_DC[1][1][2][1]==1&&multi_DC[1][2][0][0]==1&&multi_DC[1][2][0][1]==1&&multi_DC[1][2][1][0]==1&&multi_DC[1][2][1][1]==1&&multi_DC[1][2][2][0]==1&&multi_DC[1][2][2][1]==1", "col")
	...
	const double mainBeam = treeTrack->GetEntries("dsca11==0&&A0&&B0&&C0&&dE1>1.50&&aoz[0]>0.&&dsca4==1");
	const double bullsEye = treeTrack->GetEntries("dsca11==0&&A0&&B0&&C0&&O18&&dsca4==1");
